...
Seven methods:
  ...
  HE whitened kernel             (HE_whitened_kernel_e2e)  [optional dense]
...
for _p in (str(_AGENT_ROOT), str(_HERE.parent), str(_AGENT_ROOT / "experiments" / "IDP")):
...
def bench_he_whitened_kernel_e2e(*, preset, feature_dir: Path, p: int,
                                   verbose_progress: bool = True) -> dict:
    """No PCA/QR. Residualize Y, build P = cov(Y_resid) (k x k, k = p traits), whiten
    Y_resid by P^-1 to form the kernel K = Y_resid @ P^-1 @ Y_resid^T (n x n), then run
    one closed-form Haseman-Elston regression of K's off-diagonal against the GRM's
    off-diagonal: sum_h2 = (GRM_offdiag . K_offdiag) / (GRM_offdiag . GRM_offdiag).
    By linearity of OLS this equals the SUM of per-trait HE regression h2 over all k
    P-whitened (decorrelated, unit-variance) trait directions — computed in one
    O(n^2 k) pass instead of k separate per-component regressions."""
    _, _, _, _ensure_sample_ids, _, _norm_iid, _ = _imports()
    sample_ids_path = Path(_ensure_sample_ids(preset, []))
    sample_ids = {_norm_iid(l) for l in open(sample_ids_path) if l.strip()} - {""}
    t0 = time.perf_counter(); r0 = resource.getrusage(resource.RUSAGE_SELF)
    checkpoints: list = []; tag = f"he-whitened-kernel p={p}"
    GRM, df_id = load_grm_ids(preset.grm_prefix)
    _checkpoint(t0, checkpoints, 15.0, "load_grm")
    Y_raw = load_Y_from_feature_dir(feature_dir, df_id, sample_ids, p)
    _checkpoint(t0, checkpoints, 35.0, "load_feature_csv")
    Ym, _eids, covar_df, indices = align_Y_to_covariates(Y_raw, df_id, sample_ids)
    Y_resid = residualize_raw_traits(Ym, covar_df); del Y_raw, Ym
    _checkpoint(t0, checkpoints, 50.0, "residualize_raw_traits")
    n, k = Y_resid.shape
    P = (Y_resid.T @ Y_resid) / float(n - 1)
    ridge = float(max(1e-12, np.trace(P) / max(k, 1) * 1e-6))
    P = P + ridge * np.eye(k, dtype=np.float64)
    _checkpoint(t0, checkpoints, 62.0, "build_P_covariance")
    L = np.linalg.cholesky(P)
    Z = np.linalg.solve(L, Y_resid.T).T  # n x k; Z @ Z.T == Y_resid @ P^-1 @ Y_resid.T
    del Y_resid
    _checkpoint(t0, checkpoints, 72.0, "whiten_by_Pinv")
    K = Z @ Z.T  # n x n whitened phenotype kernel
    del Z
    _checkpoint(t0, checkpoints, 85.0, "build_kernel_K")
    GRM_sub = GRM[np.ix_(indices, indices)]
    triu = np.triu_indices(n, k=1)
    g_off = GRM_sub[triu]
    k_off = K[triu]
    denom = float(np.dot(g_off, g_off))
    sum_h2 = float(np.dot(g_off, k_off) / denom) if denom > 0 else None
    _checkpoint(t0, checkpoints, 100.0, "he_regression_on_kernel")
    if verbose_progress:
        print(f"[{tag}] 100% done  wall={time.perf_counter()-t0:.1f}s  sum_h2={sum_h2}", flush=True)
    wall = time.perf_counter() - t0; r1 = resource.getrusage(resource.RUSAGE_SELF)
    return {"wall_sec": round(wall, 6), "cpu_user_sec": round(r1.ru_utime-r0.ru_utime, 6),
            "cpu_sys_sec": round(r1.ru_stime-r0.ru_stime, 6), "max_rss_mb": _peak_rss_mb(),
            "n_samples": int(n), "n_traits": int(k), "residualized_raw_traits": True,
            "sum_h2": sum_h2, "timing_checkpoints_pct": checkpoints}
...
def _worker_main(kind: str, payload: dict) -> None:
    ...
    elif kind == "he_whitened_kernel":
        out = bench_he_whitened_kernel_e2e(preset=preset, feature_dir=feature_dir, p=p,
            verbose_progress=verbose)
    ...
    try:
        for p in p_values:
            ...
            if p <= skip_dense:
                ...
                row["HE_whitened_kernel_e2e"] = (
                    _run_bench_isolated("he_whitened_kernel", base_payload, label=f"whitened-kernel p={p}")
                    if isolate else bench_he_whitened_kernel_e2e(preset=preset, feature_dir=fdir, p=p, verbose_progress=vp))
            else:
                why = f"p={p} > skip_dense_above_p={skip_dense}"
                row["tr_PinvG_e2e"] = _skipped(why); row["SVD_PinvG_e2e"] = _skipped(why)
                row["HE_whitened_kernel_e2e"] = _skipped(why)
...
_SAMPLE_METHODS = ["HE_pipeline_PCA_capped_e2e",
                   "REML_PCA_capped_e2e", "tr_PinvG_e2e", "SVD_PinvG_e2e",
                   "blockwise_mvGREML_trG_trP_e2e", "HE_whitened_kernel_e2e"]
...
def run_sample_scaling(args) -> None:
    ...
    for label, ids in sample_sets:
        ...
        for method in _SAMPLE_METHODS:
            ...
            try:
                if method == "HE_pipeline_PCA_capped_e2e":
                ...
                elif method == "HE_whitened_kernel_e2e":
                    row[method] = bench_he_whitened_kernel_e2e(preset=preset, feature_dir=feature_dir, p=P_FIXED)
...
def main() -> None:
    ...
    ap.add_argument("--use-hereg", action="store_true",
                    help="Use GCTA --HEreg (with --covar/--qcovar) for the HE PCA pipeline.")
