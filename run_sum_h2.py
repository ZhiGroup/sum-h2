#!/usr/bin/env python3
"""
IDP heritability pipeline — GCTA HE-regression / REML (PCA on raw features).

PCA on RAW phenotype (no residualization) → GCTA --HEreg or --reml.
GCTA handles covariate projection via --covar / --qcovar.

Default GRM: repo_local or grm_subset_king king_over4p5_gcta_discovery (n = 2,158);
fallback sample_selection/pseudo/ (PSEUDO demo, fake IDs only).
See sample_selection/README.md for the KING filter rule.

Examples:
  python run_sum_h2.py --phenotype_csv /path/to/feature_dir/
  python run_sum_h2.py --phenotype_csv /path/to/merged.csv --output_root /out --h2-method both
"""
...
# Paths
# ---------------------------------------------------------------------------
_script_dir = Path(__file__).resolve().parent          # AGENT/repo
_agent_root = _script_dir.parent                       # AGENT
...
# Prefer real local outputs; last fallback is committed PSEUDO demo (fake IDs).
_GRM_CANDIDATES = (
    _agent_root / "repo_local" / "sample_selection",
    _agent_root / "grm_subset_king",
    _script_dir / "sample_selection",
    _script_dir / "sample_selection" / "pseudo",
)
GRM_KING_DIR = next(
    (p for p in _GRM_CANDIDATES if (p / "king_over4p5_gcta_discovery.grm.id").is_file()),
    _GRM_CANDIDATES[-1],
)
if "pseudo" in GRM_KING_DIR.parts:
    print(
        "[IDP-HEREG] Using PSEUDO demo GRM/keep lists "
        f"({GRM_KING_DIR}) — fake IDs only; not for scientific results."
    )
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "IDP heritability pipeline: PCA on raw features → GCTA --HEreg or --reml "
    ...
    parser.add_argument("--n_pca", type=int, default=128, help="Max PCA components (default 128).")
    ...
    parser.add_argument(
        ...
        help="Demean and z-score each feature column before PCA (no covariate residualization).",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--dim-reduction", choices=["pca"], default="pca",
        help="Dimensionality reduction: randomized PCA to --n_pca components (default).",
    )
    parser.add_argument(
...
def run_king_hereg(args: argparse.Namespace) -> None:
    ...
    if dim_reduction != "pca":
        raise ValueError(f"Unsupported --dim-reduction {dim_reduction!r}; only 'pca' is available.")
    ...
    # PCA Feature_*.csv always go under pca_features/ (or --pca-features-dir).
    ...
    permanent_pca_dir = features_dir
